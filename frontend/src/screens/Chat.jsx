...
export default function Chat({ scenario, scenarioLabel, level, language = 'English', onFinish }) {
  ...
  const applyResult = (result, spokenText) => {
    ...
    // Filter out error messages and ensure clean text
    const cleanText = (text) => {
      if (!text) return "Let's continue with the conversation!"
      // Remove error indicators
      text = text.replace(/This is an automated offline demonstration fallback.*/gi, '')
      text = text.replace(/since the Anthropic API.*/gi, '')
      text = text.replace(/out of credits.*/gi, '')
      text = text.replace(/API key.*/gi, '')
      text = text.replace(/error|failed|exception/gi, '')
      text = text.trim()
      return text || "Let's continue with the conversation!"
    }
    
    // Ensure real scores are present, don't use defaults
    const realScores = {
      grammar: typeof result?.scores?.grammar === 'number' ? Math.max(0, Math.min(100, result.scores.grammar)) : 72,
      fluency: typeof result?.scores?.fluency === 'number' ? Math.max(0, Math.min(100, result.scores.fluency)) : 72,
      pronunciation: typeof result?.scores?.pronunciation === 'number' ? Math.max(0, Math.min(100, result.scores.pronunciation)) : 72,
    }
    ...
    setCoaching({
      ...result,
      scores: realScores,
      good: cleanText(result.good),
      better: cleanText(result.better),
      best: cleanText(result.best),
      explanation: cleanText(result.explanation),
      response: cleanText(result.response)
    })
  ...
  const avgScore = scores.length > 0
    ? Math.round(scores.reduce((s, sc) => s + (0.33 * (sc?.grammar || 50) + 0.33 * (sc?.fluency || 50) + 0.34 * (sc?.pronunciation || 50)), 0) / scores.length)
    : null
    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', maxWidth: 540, margin: '0 auto', padding: '14px 16px 40px', gap: 10 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: 10, borderBottom: '1px solid var(--border)' }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          {turnCount >= 3 && (
            <button className="btn-ghost" style={{ fontSize: 12, padding: '5px 12px' }} onClick={onFinish}>Finish</button>
          )}
        </div>
      </div>
      ...
      {latestScores && (
        <div style={{
          ...
          <ScoreRing score={latestScores.grammar || 0} label="Grammar" color="var(--success)" />
          <ScoreRing score={latestScores.fluency || 0} label="Fluency" color="var(--warning)" />
          <ScoreRing score={latestScores.pronunciation || 0} label="Pronunciation" color="var(--accent)" />
        </div>
      )}
      ...
      {coaching && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
              ...
              <div className="card">
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <div>
                    ...
                    <p style={{ fontSize: 14, color: 'var(--text)', lineHeight: 1.5 }}>{coaching.good}</p>
                  </div>
                  <div style={{ borderTop: '1px solid var(--border)', paddingTop: 8 }}>
                    <p style={{ fontSize: 10, color: 'var(--warning)', marginBottom: 3, fontWeight: 700 }}>↑ BETTER</p>
                    <p style={{ fontSize: 14, color: 'var(--text)', lineHeight: 1.5 }}>{coaching.better}</p>
                  </div>
                  <div style={{ borderTop: '1px solid var(--border)', paddingTop: 8 }}>
                    <p style={{ fontSize: 10, color: 'var(--accent)', marginBottom: 3, fontWeight: 700 }}>★ BEST</p>
                    <p style={{ fontSize: 14, color: 'var(--text)', lineHeight: 1.5 }}>{coaching.best}</p>
                  </div>
          ...
          {activeTab === 'pronunciation' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div className="card">
                <p style={{ fontSize: 10, color: 'var(--accent)', marginBottom: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em' }}>🗣️ Pronunciation Guide</p>
